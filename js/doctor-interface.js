...
let display = function(doctors){
  ...
  if(doctors.length > 0){
    doctors.forEach(function(doctor){
      $('#doctors').append(
        `<div class='card bg-secondary mb-3'><div class='card-body'> ${doctor.title} ${doctor.first_name} ${doctor.last_name}, ${doctor.accepting_patients ? 'Currently accepting patients' : 'Currently not accepting patients'}<br>${doctor.city}, ${doctor.state}, ${doctor.street}, ${doctor.zip}<br>${doctor.specialty}<br>${doctor.website ? '<a href="' + doctor.website + '">' + doctor.website + '</a><br>' : ''}Phone: ${doctor.phone}</div></div>`
      );
    });
  }
  else {
